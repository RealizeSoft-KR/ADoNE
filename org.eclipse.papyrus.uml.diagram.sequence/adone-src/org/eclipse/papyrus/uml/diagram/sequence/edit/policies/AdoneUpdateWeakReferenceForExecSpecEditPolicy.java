...
/**
 * This class extends UpdateWeakReferenceForExecSpecEditPolicy with the intent to refine the handling of weak references
 * for ExecutionSpecifications. However, upon review, it has been identified that the specific functionalities offered
 * may not be critically necessary for the broader editing framework. Consequently, this policy is earmarked for a period
 * of further observation to assess its impact and utility more thoroughly. Based on the outcomes of this evaluation,
 * the class might be considered for deprecation, aiming to maintain an optimized and streamlined set of edit policies
 * without diluting the core functional requirements.
 */
public class AdoneUpdateWeakReferenceForExecSpecEditPolicy extends UpdateWeakReferenceForExecSpecEditPolicy {
	...
	private Command getUpdateWeakRefForExecSpecResize(final ChangeBoundsRequest request) {
		...
		if (moveDelta.y != 0 && mustMove) {
			...
			// 우선 대상 Bes 옮기자 (2023-11-22) => 이미 Selected EP 는 처리된 후임. 여기서는 Weak ref 에 해당하는 Msg || BES 처리 => 수정 필요 없음 (2023-11-24)

			// EditPart targetEp = (EditPart) request.getEditParts().get(0);
			//
			// if (!SenderRequestUtils.isASender(request, getHost())) {
			// UpdateWeakReferenceEditPolicy.moveRoundedEditPart(getHost(), moveDelta, compoundCommand, targetEp, null);
			// }

			// 우선 대상 Bes 옮기자 (2023-11-22)
