 ...
 *   RealizeSoft - Adjusted routing logic to correctly handle recursive messages,
 *   ensuring accurate display and positioning of self-invocation messages in sequence diagrams.
 *****************************************************************************/
...
/**
 * The AdoneMessageRouter class extends the ObliqueRouter and copies codes from
 * org.eclipse.papyrus.uml.diagram.sequence.draw2d.routers.MessageRouter to provide specialized routing
 * capabilities for UML sequence diagrams within the Adone modeling environment. This router
 * is designed to handle different kinds of message routing, including horizontal, oblique, and
 * self-messages, ensuring that messages are displayed clearly and accurately on the diagram.
 *
 * This router implements a custom algorithm to determine the kind of routing needed based on
 * the connection and the points involved in the routing process. It supports rectilinear routing
 * for horizontal and self-messages, and maintains oblique routing for other cases, providing
 * flexibility in how messages are represented.
 *
 * Notably, modifications were made to this class to specifically address and resolve issues related
 * to the routing of recursive messages. These modifications include adjustments to the routing
 * algorithm to ensure that recursive messages are correctly displayed with appropriate bend points,
 * thereby enhancing the readability and accuracy of sequence diagrams. The changes are carefully
 * integrated into the existing routing logic to maintain the overall integrity and functionality
 * of the message routing process.
 */
@SuppressWarnings({ "restriction", "deprecation" })
public class AdoneMessageRouter extends ObliqueRouter {
	...
	public static enum RouterKind {
		...
		private static boolean isSelfConnection(Connection conn) {
			...
			while (sourceLifeline != null && !(sourceLifeline instanceof AdoneLifelineNodePlate)) {
				sourceLifeline = sourceLifeline.getParent();
			}
			IFigure targetLifeline = conn.getTargetAnchor().getOwner();
			while (targetLifeline != null && !(targetLifeline instanceof AdoneLifelineNodePlate)) {
	...
	public void routeLine(Connection conn, int nestedRoutingDepth, PointList newLine) {
		...
		case SELF:
			...
			if (checkSelfRelConnection(conn, newLine)) {
				// 아래 resetEndPointsToEdge 메소드 실행 시 메시지 우측으로 강제 이동 처리 안되어 주석처리 (2024-01-29)
	...
	protected void rectilinearResetEndPointsToEdge(Connection conn, PointList line) {
		...
		if (line.size() == 0) {
			...
			if (conn.getSourceAnchor() instanceof OrthogonalConnectionAnchor) {
				...
				if (RouterKind.getKind(conn, line).equals(RouterKind.SELF)) {
					// 재귀호출 메시지의 시작점과 종료점을 우측으로 강제 이동 처리 (2024-01-29)
					// 7 이상 넘어가면 라인 망가짐 (2024-01-30)
					Point p = OrthogonalRouterUtilities.getOrthogonalLineSegToAnchorLoc(conn, conn.getSourceAnchor(), start).getOrigin();
					p.x += 7;
					line.insertPoint(p, 0);
			...
			if (conn.getTargetAnchor() instanceof OrthogonalConnectionAnchor) {
				...
				if (RouterKind.getKind(conn, line).equals(RouterKind.SELF)) {
					Point p = OrthogonalRouterUtilities.getOrthogonalLineSegToAnchorLoc(conn, conn.getTargetAnchor(), end).getOrigin();
					// 7 이상 넘어가면 라인 망가짐 (2024-01-30)
					p.x += 7;
					line.addPoint(p);
			...
			sourceAnchorRelativeLocation = getAnchorOffRectangleDirection(line.getFirstPoint(), source);
			if (RouterKind.getKind(conn, line).equals(RouterKind.SELF)) {
				// sourceAnchorRelativeLocation = PositionConstants.WEST;
			}
			offSourceDirection = getOffShapeDirection(sourceAnchorRelativeLocation);
			targetAnchorRelativeLocation = getAnchorOffRectangleDirection(line.getLastPoint(), target);
			if (RouterKind.getKind(conn, line).equals(RouterKind.SELF)) {
				// targetAnchorRelativeLocation = PositionConstants.WEST;
			}
			offTargetDirection = getOffShapeDirection(targetAnchorRelativeLocation);
		}
		/*
