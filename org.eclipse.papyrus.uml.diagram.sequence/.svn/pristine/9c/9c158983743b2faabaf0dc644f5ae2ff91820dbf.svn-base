 ...
 *   RealizeSoft - Modifided to improve functionality and provide detailed, relevant tooltips
 *****************************************************************************/
...
/**
 * This class is a redefined and enhanced version of a previously established utility class for managing tooltips within sequence diagrams.
 * It includes a comprehensive set of methods largely copied from the original class but with modifications and improvements specifically
 * tailored to enhance tooltip functionality. The redefinitions within this class focus on providing more detailed and contextually relevant
 * tooltips for various UML elements, thereby improving user interaction and understanding of sequence diagrams.
 */
public class AdoneToolTipUtil {
	...
	static {
		...
		// Added support for tooltips on InteractionOperand's Constraint (2024-01-28)
		ELEMENT_SUPPORT_TOOLTIPS.add(UMLPackage.Literals.INTERACTION_CONSTRAINT);
	}
	...
	public static class EditPartTooltipUpdater implements TooltipUpdater {
		...
		public String getTooltipDescription() {
			...
			// Removed unnecessary parent information from Lifeline tooltip content (2024-01-28)
			...
			if (semanticElement instanceof CombinedFragment && !((editPart instanceof CCombinedFragmentEditPart) && ((CCombinedFragmentEditPart) editPart).isCoregion())) {
				...
				// Removed unnecessary parent information from InteractionOperand tooltip content (2024-01-28)
			...
			} else if (semanticElement instanceof InteractionConstraint) {
				// Updated to display all conditions written for Interaction Operand Guard in the tooltip (2024-02-04)
				InteractionConstraint constraint = (InteractionConstraint) semanticElement;
				descBuf.append("Guard Condition: \n");
				descBuf.append("---------------------------------------\n");
				if (constraint.getSpecification() != null) {
					String spec = constraint.getSpecification().stringValue();
					descBuf.append(spec);
				} else {
					descBuf.append("");
				}
				...
				// Updated to display signature operation documentation in message tooltips (2024-02-04)
				Message msg = (Message) semanticElement;

				if (msg.getSignature() != null) {
					Operation op = (Operation) msg.getSignature();
					String doc = AdoneUMLCommonHelper.getDocument(op);
					descBuf.append("Operation Document: \n");
					descBuf.append("---------------------------------------\n");
					descBuf.append(doc);
					descBuf.append("\n---------------------------------------");
				}
